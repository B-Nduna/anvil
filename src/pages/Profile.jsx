...
import { Stepper, SegmentedControl, Slider } from '../ui/Controls';
...
export default function Profile({ data, setData, notify, onReset }) {
  ...
  return <section className="page">
    ...
    <Card title="Identity & baseline">
      <div className="formGrid"><label>Name<input value={p.name} onChange={e => set('name', e.target.value)} /></label></div>
      <div className="touchGrid">
        <Stepper label="AGE" value={Number(p.age) || 0} onChange={v => set('age', v)} step={1} min={10} max={100} />
        <Stepper label="WEIGHT" value={Number(p.weight) || 0} onChange={v => set('weight', v)} step={0.5} min={30} max={250} unit=" KG" />
        <Stepper label="HEIGHT" value={Number(p.height) || 0} onChange={v => set('height', v)} step={1} min={100} max={230} unit=" CM" />
      </div>
      <div className="onboardSubgroup"><span className="onboardSubLabel">Body fat % <em>(optional)</em></span>
        <input type="number" className="onboardTextarea" value={p.bodyFatPct} onChange={e => set('bodyFatPct', e.target.value === '' ? '' : Number(e.target.value))} placeholder="Not set" />
      </div>
    ...
    <Card title="Experience">
      <div className="touchGrid"><Stepper label="TRAINING AGE" value={Number(p.trainingAge) || 0} onChange={v => set('trainingAge', v)} step={0.5} min={0} max={40} unit=" YRS" /></div>
    ...
    <Card title="Program shape">
      ...
      <div className="onboardSubgroup"><span className="onboardSubLabel">Frequency (days/week)</span>
        <SegmentedControl value={p.frequency} onChange={v => set('frequency', v)} options={[1, 2, 3, 4, 5, 6, 7]} />
      </div>
      <div className="onboardSubgroup">
        <Slider label="SESSION LENGTH" value={Number(p.sessionDuration) || 60} onChange={v => set('sessionDuration', v)} min={15} max={120} step={5} unit=" min" marks={[15, 45, 75, 120]} />
      </div>
    </Card>
